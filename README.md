...
The following diagram represents the private CAs and the VPN client certificate that you’ll need to create in your AWS environment. It also shows the tunnel-specific certificates that will be automatically generated when you configure the site-to-site VPN connection.

<img src="images/cert-based-authentication-aws-environment.png" alt="Certificate and key data required" width="800"/>

#### Create private certificates authorities (CAs)
When using certificate-based authentication, you’ll need to use the AWS Certificate Manager private CA feature to create root and subordinate private CAs. The root CA is used to sign the subordinate CA while the subordinate CA is used to sign certificates used to support your site-to-site VPN connections.

#### Create VPN client certificate
Once you’ve created the necessary CAs, you’ll use AWS Certificate Manager to create a client certificate to be used to authenticate your on-premises VPN gateway when it initiates the VPN connection.

The VPN client certificate is signed by the subordinate CA.

When you configure a customer gateway in your AWS environment, you’ll specify certificate-based authentication and associate your VPN client certificate with the customer gateway. 

Later, when your VPN connection and tunnels are established, this association ensures that only a customer gateway that has this VPN client certificate can connect to your AWS environment.

#### Export public certificates and VPN client private key
Once you’ve created the private CAs and the VPN client certificate, you’ll need to save copies of the associated public certificates to your desktop copies. 

The public certificates are needed in your simulated on-premises environment so that the strongSwan VPN tool can validate the authenticity of both the VPN client certificate and tunnel-specific certificates that are sent to the client when the VPN connection is established.

You’ll also need to save a copy of VPN client private key so that the strongSwan VPN tool can decrypt the content of the VPN client private key. 

Since the VPN client private key is sensitive data, you’ll need to specify a passphrase to be used to encrypt the private key file.

#### Note the domain name of tunnel-specific certificates

The tunnel-specific certificates are automatically generated by AWS when you either create a transit gateway VPN attachment or create a site-to-site connection.  These certificates contain a domain name that you’ll need to take note of and supply when you deploy the strongSwan VPN gateway stack in your simulated on-premises environment. 

When the tunnels are being established, the srongSwan tool will use the domain names to help validate the tunnel-specific certificates exchanged when the tunnels are established.
...
First, you’ll need to upload the public certificates and VPN client private key to an S3 bucket that is accessible from your simulated on-premises environment.

Next, you’ll also need to create a secret in AWS Secrets Manager and store the passphrase to decrypt the VPN client private key in that secret. 

The following diagram shows the public certificates and the VPN client private key stored in an S3 bucket. The passphrase for the VPN client private key is stored in a secret in AWS secrets manager.
